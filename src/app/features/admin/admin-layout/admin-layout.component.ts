  ...
  template: `
    <div class="flex h-screen overflow-hidden" style="background: var(--color-bg);">
      ...
      @if (sidebarOpen() && isMobile()) {
        <div class="fixed inset-0 z-30 bg-black/30" (click)="sidebarOpen.set(false)"></div>
      }
      ...
      <aside
        ...
        style="background: white; border-right: 1px solid var(--color-border); box-shadow: 2px 0 15px rgba(0,0,0,0.04);"
          ...
          <a routerLink="/" class="flex items-center gap-2.5 min-w-0" aria-label="Happy Hamper Home">
            ...
            @if (!collapsed()) {
              <div class="min-w-0 sidebar-label">
                ...
                <div class="text-[10px] font-semibold uppercase tracking-widest px-1.5 py-0.5 rounded mt-0.5 inline-block" style="background: var(--color-primary-light); color: var(--color-primary-dark);">Admin</div>
              </div>
            }
          </a>
        </div>
          ...
          @for (item of navItems; track item.path) {
            ...
            >
              ...
              @if (!collapsed()) {
                ...
                @if (item.badge) {
                  <span class="ml-auto text-xs px-1.5 py-0.5 rounded-full font-bold text-white" style="background: var(--color-orange);">{{ item.badge }}</span>
                }
              }
              <!-- Tooltip when collapsed -->
              @if (collapsed()) {
                <div class="absolute left-full ml-2 px-2.5 py-1 rounded-lg text-xs font-medium whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50 shadow-md"
                     style="background: #2D2D2D; color: white;">
                  {{ item.label }}
                  @if (item.badge) {
                    <span class="ml-1 px-1 py-0.5 rounded text-white text-[10px] font-bold" style="background: var(--color-orange);">{{ item.badge }}</span>
                  }
                </div>
              }
            </a>
      ...
      <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <!-- Top Header Bar -->
        <header class="flex-shrink-0 flex items-center justify-between px-6 py-4 bg-white"
                style="border-bottom: 1px solid var(--color-border); box-shadow: 0 1px 8px rgba(0,0,0,0.04);">
          <!-- Mobile hamburger -->
          <button
            class="lg:hidden btn-icon mr-3"
            (click)="toggleSidebar()"
          ...
          <!-- Breadcrumb -->
          <nav class="breadcrumb hidden sm:flex" aria-label="Breadcrumb">
            <a routerLink="/admin" class="hover:text-primary-600 transition-colors">Admin</a>
            <svg class="w-4 h-4 breadcrumb-separator mx-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
            </svg>
            <span style="color: var(--color-primary); font-weight: 500;">Dashboard</span>
          </nav>
          ...
          <div class="flex items-center gap-3 ml-auto">
            ...
            <button class="btn-icon relative" aria-label="Notifications">
              ...
              <span class="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-white" style="background: var(--color-orange);"></span>
            </button>
            ...
            <div class="flex items-center gap-2.5">
              ...
              <div class="hidden sm:block">
                <div class="text-sm font-semibold" style="color: #2D2D2D;">Admin</div>
                <div class="text-xs" style="color: #9CA3AF;">Administrator</div>
              </div>
            </div>
          </div>
        </header>
...
export class AdminLayoutComponent {
  ...
  navItems = [
    ...
    { path: '/admin/orders',     label: 'Orders',     badge: '12',  exact: false, svgPath: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01' },
    ...
    { path: '/admin/inventory',  label: 'Inventory',  badge: '3',   exact: false, svgPath: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4' },
    ...
    { path: '/admin/returns',    label: 'Returns',    badge: '7',   exact: false, svgPath: 'M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15' },
  ];
}
